...
{
for(pos = 90; pos <= 140; pos += 1){    // goes from 90 degrees to 180 degrees:
...
if((distance > 6) && (distance < 20)){            //check wheather the ultrasonic sensor's value stays between 1 to 15.
...
}else if((distance > 1) && (distance <5)){            //check wheather the ultrasonic sensor's value stays between 1 to 15.
                                                  //If the condition is 'true' then the statement below will execute:
  //Move Forward:
  Motor3.setSpeed(130);  //define motor3 speed:
  ...
  Motor4.run(FOREWARD);  //rotate motor4 anticlockwise:
  delay(150);
